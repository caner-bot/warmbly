...
The separation is crossed in exactly one direction, and never silently: when the premium tier runs thin (fewer than 25 other eligible recipients), it borrows free-tier mailboxes that have been members for at least three days and are healthy now, prefers a fresh partner from its own tier before a borrowed one, and checks each borrowed mailbox against its own pool's standing before using it. The free tier never borrows premium mailboxes, so free or restricted traffic does not reach paying inboxes.
